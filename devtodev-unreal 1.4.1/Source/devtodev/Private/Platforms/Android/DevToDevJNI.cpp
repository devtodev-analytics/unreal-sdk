...
namespace devtodev {
    ...
    void jni_setActiveLog(bool isActive) {
        ...
        if (jClass) {
            jmethodID jMethod = env->GetStaticMethodID(jClass, "setLogLevel", "(Lcom/devtodev/core/utils/log/LogLevel;)V");
            if (jMethod) {
                jclass logLevelClass = FAndroidApplication::FindJavaClass("com/devtodev/core/utils/log/LogLevel");
                if (logLevelClass) {
                    jfieldID logLevelVerboseField = env->GetStaticFieldID(logLevelClass , "Verbose", "Lcom/devtodev/core/utils/log/LogLevel;");
                    if (logLevelVerboseField) {
                        jobject infoObjectField = env->GetStaticObjectField(logLevelClass, logLevelVerboseField);
                        if (infoObjectField) {
                            env->CallStaticVoidMethod(jClass, jMethod, infoObjectField);
                            env->DeleteLocalRef(infoObjectField);
                        } else {
                            __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "*** Failed to get LogLevel enum field value.");
                        }
                    } else {
                        __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "*** Failed to get LogLevel enum field id.");
                    }
                    env->DeleteLocalRef(logLevelClass);
                } else {
                    __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "*** Failed to get LogLevel enum class.");
                }
            } else {
                __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "*** Failed to get 'setLogLevel' method.");
            }
            env->DeleteLocalRef(jClass);
        } else {
            __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "*** Failed to get 'CoreLog' class.");
        }
    }
