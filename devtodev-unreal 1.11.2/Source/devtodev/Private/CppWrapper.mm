...
#if PLATFORM_IOS
#import <devtodev/devtodev.h> // analytics obj-c interface
...
namespace devtodev
{
    ...
    void DevToDev::EnablePushNotifications()
    {
#if PLATFORM_IOS
        [[DTDPushDelegateProxy sharedProxy] onRegisterForPushNotifications];
        
        id pushManager = [::DevToDev pushManager];
        [pushManager setPushNotificationsEnabled: YES];
    ...
    void DevToDev::StartProgressionEvent(const std::string& locationName, const std::map<std::string, std::string>& params)
    {
#if PLATFORM_IOS
        std::map<std::string, std::string> earned;
        std::map<std::string, std::string> spent;

        LocationEventParams * startParams = transformLocationEventParams(params, earned, spent);
        [::DevToDev startProgressionEvent:transform(locationName) withParameters:startParams];
#endif
    }

    void DevToDev::EndProgressionEvent(const std::string& locationName, const std::map<std::string, std::string>& params, const std::map<std::string, std::string>& earned, const std::map<std::string, std::string>& spent)
    {
#if PLATFORM_IOS
        LocationEventParams * endParams = transformLocationEventParams(params, earned, spent);
        [::DevToDev endProgressionEvent:transform(locationName) withParameters:endParams];
#endif
    }
